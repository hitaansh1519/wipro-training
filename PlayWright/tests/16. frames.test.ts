...
test('Frame1', async({ page })=> {
    ...
    await frameEle.locator('#RESULT_TextField-3').fill('Bathinda')
    await frameEle.locator('#RESULT_TextArea-5').fill('This is the test for Text Area.')
    await page.pause();
})
...
test('Frame2', async({ page })=> {
    ...
    const frameEle1: FrameLocator = page.frameLocator('div > iframe#pact1');
    await frameEle1.getByPlaceholder('First Crush').fill('X')

    const frameEle2: FrameLocator = frameEle1.frameLocator('#pact2');
    await frameEle2.getByPlaceholder('Current Crush Name').fill('N')

    const frameEle3: FrameLocator = frameEle2.frameLocator('#pact3');
    await frameEle3.getByPlaceholder('Destiny').fill('B')
...
test('Frame3', async ({page}) => {
    ...
    const frame1: FrameLocator = page.frameLocator('div > iframe#pact1')
    await frame1.locator('#inp_val').fill('X')

    const frame2: FrameLocator = frame1.frameLocator('#pact2')
    await frame2.locator('#jex').fill('N')

    const frame3: FrameLocator = frame2.frameLocator('#pact3')
    await frame3.locator('#glaf').fill('XYZ')
